...
if [[ "${#trimmed}" -gt "$maxlen" ]]; then
  audit too_long
  exit 0
fi

# Gate 2: negation guard — "not done yet", "还没完成", "未完成", "差不多".
if printf '%s' "$lower" | grep -Eiq "not (done|finished|ready)|isn'?t done|haven'?t|n'?t done yet"; then
  audit negation
  exit 0
fi
if printf '%s' "$trimmed" | grep -Eq "没(做完|完成|搞定|弄好)|还没|未完成|差点|没好"; then
  audit negation_zh
  exit 0
fi
...
human_size() { # $1 = path
  local b
  b="$(stat -f '%z' "$1" 2>/dev/null || stat -c '%s' "$1" 2>/dev/null)"
  [[ -z "$b" ]] && {
    printf '?'
    return
  }
